...
# Deploy-CertificateToAppliances.ps1
...
function Get-SecureCredential {
    param(
        [Parameter(Mandatory = $true)]
        [hashtable]$ApplianceConfig
    )

    if ($ApplianceConfig.EncryptedPassword) {
        # Use encrypted standard string
        $securePassword = ConvertTo-SecureString $ApplianceConfig.EncryptedPassword
        return New-Object System.Management.Automation.PSCredential($ApplianceConfig.Username, $securePassword)
    } else {
        # Interactive credential prompt for security
        $cred = Get-Credential -UserName $ApplianceConfig.Username -Message "Enter credentials for $($ApplianceConfig.Username)"
        return $cred
    }
}

function Deploy-ToNetScaler {
    $credential = Get-SecureCredential -ApplianceConfig $config.NetScaler

    $credential = Get-SecureCredential -credConfig $config.NetScaler
