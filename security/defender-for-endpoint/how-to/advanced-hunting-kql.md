...
Advanced Hunting operates on data already ingested by MDE. It cannot report on devices that have never communicated with the service, and it cannot confirm local registry or service state. For local device validation, see the [PowerShell tutorial](../tutorials/powershell-validation.md).
...
Results can be exported to Excel (display limit: 100,000 rows per query). For programmatic access to the same data, use the [Graph API tutorial](../tutorials/graph-api-validation.md).
...
- [Azure Workbook for MDE](../workbooks/README.md)
