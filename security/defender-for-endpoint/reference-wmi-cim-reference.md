...
For Windows 10/11 and Server 2016+, prefer [PowerShell validation](tutorial-powershell-validation.md) using `Get-MpComputerStatus` — it is faster, more complete, and does not require namespace or class name knowledge.
...
1. **Modern Windows (10/11 or Server 2016+) with the Defender module available** — Use [PowerShell validation](tutorial-powershell-validation.md) with `Get-MpComputerStatus` instead; it is faster and more complete
2. **Organisation-wide reporting** — Use [Graph API validation](tutorial-graph-api-validation.md), which does not require per-device network connectivity
...
- [PowerShell validation tutorial](tutorial-powershell-validation.md)
