...
.NOTES
    ...
    - Thread-safe logging using synchronised collections (PS7+) or sequential writes (PS5.1)
    ...
    try {
        ...
        if ($Result.ValidationMethod -eq 'None') {
            ...
            if ($Result.ValidationMethod -in @('CIM-WSMan', 'WMI-DCOM')) {
                # Get onboarding state, tenant identity, onboarding policy, and service persistence
                # from registry (CIM/WMI doesn't provide any of these)
                ...
                if ($RegData.Success -and $RegData.Data) {
                    ...
                    # Tenant Identity
                    if ($RegData.Data.OrgId) {
                        $Result.OrgId = $RegData.Data.OrgId
                    }
                    if ($RegData.Data.SenseId) {
                        $Result.SenseId = $RegData.Data.SenseId
                    }

                    # Onboarding Policy
                    if ($RegData.Data.OnboardingBlobConfigured) {
                        $Result.OnboardingBlobConfigured = $RegData.Data.OnboardingBlobConfigured
                        $Result.OnboardingBlobLength = $RegData.Data.OnboardingBlobLength
                    }

                    # Service Persistence (start type not available from Get-ServiceStatus)
                    if ($RegData.Data.SENSEServiceStartType) {
                        $Result.SENSEServiceStartType = $RegData.Data.SENSEServiceStartType
                    }
                    if ($RegData.Data.DiagTrackServiceStartType) {
                        $Result.DiagTrackServiceStartType = $RegData.Data.DiagTrackServiceStartType
                    }
                }

                # Get service running status (CIM/WMI doesn't provide this either)
    ...
    try {
        $ScriptBlock = {
            ...
            $RegData = [PSCustomObject]@{
                ...
                OnboardingBlobConfigured = $false
                OnboardingBlobLength     = $null
                # Service Status and Persistence
                SENSEServiceStatus       = $null
                SENSEServiceStartType    = $null
                DiagTrackServiceStatus   = $null
                DiagTrackServiceStartType = $null
            ...
            # Tenant Identity: OrgId and SenseId from the Status key
            # OrgId identifies the tenant this device is onboarded to — a mismatch indicates wrong-tenant onboarding
            # SenseId is the device-level sensor identifier used for portal cross-referencing
            # Reference: https://learn.microsoft.com/en-us/defender-endpoint/troubleshoot-onboarding
            $RegData.OrgId = Get-ItemProperty $StatusPath -Name 'OrgId' -ErrorAction SilentlyContinue | Select-Object -ExpandProperty 'OrgId'
            $RegData.SenseId = Get-ItemProperty $StatusPath -Name 'SenseId' -ErrorAction SilentlyContinue | Select-Object -ExpandProperty 'SenseId'

            # Onboarding Policy: confirm the onboarding blob was pushed by policy
            # OnboardingBlobConfigured distinguishes "policy not pushed" from "push failed"
            # OnboardingBlobLength confirms the blob is non-empty and non-corrupted
            # Reference: https://learn.microsoft.com/en-us/defender-endpoint/troubleshoot-onboarding
            $PolicyPath = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows Advanced Threat Protection'
            $OnboardingBlob = Get-ItemProperty $PolicyPath -Name 'OnboardingInfo' -ErrorAction SilentlyContinue | Select-Object -ExpandProperty 'OnboardingInfo'
            if ($OnboardingBlob) {
                $RegData.OnboardingBlobConfigured = $true
                $RegData.OnboardingBlobLength = $OnboardingBlob.Length
            }

            # Check SENSE service (MDE behavioural sensor)
            # StartType = Manual means the service won't survive a reboot without intervention
            ...
            if ($SenseService) {
                ...
                $RegData.SENSEServiceStartType = $SenseService.StartType
            }
            ...
            if ($DiagService) {
                ...
                $RegData.DiagTrackServiceStartType = $DiagService.StartType
            }
...
function Merge-MDEResult {
    ...
    if ($Source.SenseId) {
        $Target.SenseId = $Source.SenseId
    }

    # Onboarding Policy
    if ($Source.OnboardingBlobConfigured) {
        $Target.OnboardingBlobConfigured = $Source.OnboardingBlobConfigured
        $Target.OnboardingBlobLength = $Source.OnboardingBlobLength
    }
    ...
    # Service Persistence: Manual start type means the service won't survive a reboot
    if ($Source.SENSEServiceStartType) {
        $Target.SENSEServiceStartType = $Source.SENSEServiceStartType
    }
    ...
    if ($Source.DiagTrackServiceStartType) {
        $Target.DiagTrackServiceStartType = $Source.DiagTrackServiceStartType
    }
...
try {
    if ($PSCmdlet.ParameterSetName -eq 'Single') {
        ...
        if ($UsesParallel) {
            ...
            # Create thread-safe synchronised list for log entries
