<#
.SYNOPSIS
    Downloads and extracts the Microsoft Defender for Endpoint Client Analyser tool.

.DESCRIPTION
    Downloads the latest MDE Client Analyser from Microsoft's official aka.ms URL,
    saves it to the script directory, and extracts it to a subfolder for immediate use.

    The analyser provides comprehensive diagnostic capabilities including:
...
.PARAMETER Force
    Force re-download even if the analyser already exists locally.
...
.REFERENCES
    Run the client analyser on Windows - Microsoft Defender for Endpoint
    https://learn.microsoft.com/en-us/defender-endpoint/run-analyzer-windows

    MDE Client Analyser - GitHub Repository
...
try {
    Write-Host '=== Microsoft Defender for Endpoint Client Analyser Downloader ===' -ForegroundColor Cyan
    ...
    # Download the analyser
    if (-not $UseExisting) {
        Write-Host 'Downloading MDE Client Analyser...' -ForegroundColor Cyan
    ...
    # Extract the analyser
    if (-not $SkipExtraction) {
        Write-Host "`nExtracting analyser..." -ForegroundColor Cyan
        ...
        # Locate the main analyser executable
        ...
        if ($AnalyzerCmd) {
            Write-Host "`n=== Analyser Ready ===" -ForegroundColor Green
            ...
            Write-Host 'To run the analyser with default settings:' -ForegroundColor White
    ...
    Write-Host "`n✅ MDE Client Analyser ready for use!" -ForegroundColor Green
    ...
    Write-Error "Failed to download/extract MDE Client Analyser: $_"
