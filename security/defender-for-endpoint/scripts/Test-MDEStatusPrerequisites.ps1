            ...
            if ($PathTest.TraceRoute) {
                ...
                if ($Hops -gt $MaxAcceptableHops) {
                    Add-TestResult -TestCase 'Test-Case-N06' -Category 'Network-Path' -TestName 'Network Path Quality' `
                        ...
                        -Remediation "1. Review traceroute path for routing loops: $($PathTest.TraceRoute -join ' -> '); 2. Check for routing inefficiencies; 3. Verify VPN routing is optimal; 4. High hop count may increase latency affecting WinRM session timeouts; 5. Consider network path optimisation; 6. Verify NetworkType parameter matches actual deployment ($NetworkType)"
                }
...
if ($OverallStatus -eq 'Pass' -and $FailedTests -eq 0) {
    ...
    if ($WarningTests -gt 0) {
        Write-Host ''
        Write-Host 'Warnings detected (non-critical):' -ForegroundColor Yellow
...
<body>
    <h1>MDE Status Prerequisites Validation Report</h1>
    <div class="summary">
...
$(if ($WarningTests -gt 0) { "        <div class='summary-item'><span class='warning'>Warnings: $WarningTests</span></div>" })
