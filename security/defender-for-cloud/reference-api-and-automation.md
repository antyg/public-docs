...
The [`config/azure-policy-initiatives.json`](../config/azure-policy-initiatives.json) file contains a custom policy initiative definition (`DefenderForCloudGovernmentBaseline`) that bundles 14 policy definitions across 8 control groups:
...
$initiativeJson = Get-Content '../config/azure-policy-initiatives.json' -Raw
